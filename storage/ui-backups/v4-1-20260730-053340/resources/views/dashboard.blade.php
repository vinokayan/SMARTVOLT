...
@section('page-subtitle', 'Pantau energi dan perangkat rumah secara real-time.')
...
@section('content')
    <section class="sv-dashboard-intro" aria-labelledby="dashboard-greeting">
        <span class="sv-dashboard-intro-icon" aria-hidden="true">
            <x-icon name="home" :size="27" />
        </span>
        ...
        <span class="sv-dashboard-intro-state {{ ($system['has_fresh_data'] ?? false) ? 'is-online' : 'is-waiting' }}" aria-hidden="true">
            <x-icon :name="($system['has_fresh_data'] ?? false) ? 'shield' : 'clock'" :size="22" />
        </span>
    </section>
    ...
    <section class="sv-dashboard-metrics" aria-label="Ringkasan penggunaan listrik">
        <article class="sv-metric-card sv-metric-card-blue">
            <div class="sv-metric-card-top">
                <span class="sv-metric-card-icon" aria-hidden="true"><x-icon name="bolt" :size="23" /></span>
                <div>
        ...
        <article class="sv-metric-card sv-metric-card-green">
            <div class="sv-metric-card-top">
                <span class="sv-metric-card-icon" aria-hidden="true"><x-icon name="energy" :size="23" /></span>
                <div>
        ...
        <article class="sv-metric-card sv-metric-card-orange">
            <div class="sv-metric-card-top">
                <span class="sv-metric-card-icon" aria-hidden="true"><x-icon name="money" :size="23" /></span>
                <div>
        ...
        <article class="sv-metric-card sv-metric-card-cyan">
            <div class="sv-metric-card-top">
                <span class="sv-metric-card-icon" aria-hidden="true"><x-icon name="plug" :size="23" /></span>
                <div>
    ...
    <section class="sv-system-strip" aria-label="Status sistem SmartVolt">
        <article class="sv-system-item {{ ($system['esp_online'] ?? false) ? 'is-success' : 'is-warning' }}" data-system-esp>
            <span class="sv-system-icon" aria-hidden="true"><x-icon name="sensor" :size="20" /></span>
        ...
        <article class="sv-system-item {{ ($system['has_fresh_data'] ?? false) ? 'is-success' : 'is-warning' }}" data-system-pzem>
            <span class="sv-system-icon" aria-hidden="true"><x-icon name="activity" :size="20" /></span>
            <div class="sv-system-copy">
        ...
        <article class="sv-system-item {{ ($system['esp_online'] ?? false) ? 'is-success' : 'is-warning' }}" data-system-control>
            <span class="sv-system-icon" aria-hidden="true"><x-icon name="mqtt" :size="20" /></span>
            <div class="sv-system-copy">
        ...
        <article class="sv-system-item {{ ($system['has_data'] ?? false) ? 'is-success' : 'is-warning' }}" data-system-latest>
            <span class="sv-system-icon" aria-hidden="true"><x-icon name="clock" :size="20" /></span>
            <div class="sv-system-copy">
        ...
        <article class="sv-card sv-chart-card">
            <header class="sv-card-header">
                <div>
                    ...
                    <p>Ringkasan perubahan daya sepanjang hari.</p>
                </div>
            ...
            <div class="sv-card-body">
                ...
                <div class="sv-empty-state {{ collect($chart['labels'] ?? [])->isNotEmpty() ? 'is-hidden' : '' }}" data-dashboard-chart-empty>
                    ...
                    <p>Grafik akan tampil saat data terbaru diterima.</p>
                </div>
        ...
        <article class="sv-card sv-control-card">
            <header class="sv-card-header">
                <div>
                    ...
                    <p>Kontrol cepat perangkat yang terhubung.</p>
                </div>
                            ...
                            <details class="sv-room-control" data-room-id="{{ $room['id'] }}" {{ $loop->first ? 'open' : '' }}>
                                <summary>
                                    <span class="sv-room-summary-main">
                                        <span class="sv-room-icon" aria-hidden="true"><x-icon name="rooms" :size="18" /></span>
                                    ...
                                    @forelse(collect($room['devices'] ?? []) as $device)
                                        ...
                                        <div class="sv-device-row" data-device-row="{{ $device['id'] }}">
                                            <div class="sv-device-main">
                                                <span class="sv-device-icon" aria-hidden="true"><x-icon name="lightbulb" :size="17" /></span>
                        ...
                        <table class="sv-table">
                            ...
                            <tbody>
                                @foreach($recentReadings as $reading)
                                    <tr>
                                        <td><span class="sv-table-room"><x-icon name="home" :size="15" />{{ $reading['room_name'] ?? '-' }}</span></td>
...
@push('scripts')
    ...
    <script src="{{ asset('assets/js/smartvolt-dashboard.js') }}?v=20260729" defer></script>
@endpush
